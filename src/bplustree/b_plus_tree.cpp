...
auto BPLUSTREE_TYPE::InsertIntoParent(BPlusTreePage *old_node,
                                      const KeyType &key,
                                      BPlusTreePage *new_node, Context *ctx)
    -> void {
  // 如果旧节点是根节点
...
void BPLUSTREE_TYPE::Remove(const KeyType &key) {
  ...
  if (leaf_page->IsSafe(OperationType::DELETE) ||
      leaf_page->GetPageId() == ctx.root_page_id_) {
    if (ctx.WSize() > 1) {
      ctx.WPopFront();
    }
    if (leaf_page->FindValue(key, comparator_, value, &delete_index)) {
    ...
    // 如果是根页面且为空，删除根页面
    if (leaf_page->GetPageId() == ctx.root_page_id_ &&
        leaf_page->GetSize() == 0) {
...
auto BPLUSTREE_TYPE::RemoveInternalEntry(InternalPage *internal_page,
  ...
  // 如果是根页面且只有一个子节点
  if (internal_page->GetPageId() == ctx->root_page_id_ &&
...
auto BPLUSTREE_TYPE::SplitInternalPage(InternalPage *internal_page,
  ...
  // 分裂节点
  new_page->CopyHalfFrom(internal_page->GetData(), split_index + 1, cur_size);
  new_page->SetSize(cur_size - split_index - 1);
  internal_page->SetSize(split_index + 1);
...
// INDEX_TEMPLATE_ARGUMENTS
// auto BPLUSTREE_TYPE::SplitLeafPage(LeafPage *leaf_page, LeafPage *new_page,
//                                    const KeyType &key, const ValueType
//                                    &value, page_id_t new_page_id) -> KeyType
//                                    {
...
//   // 找到新键的插入位置
//   auto compare_first = [this](
//                            const std::pair<KeyType, ValueType> &lhs,
...
//     all_pairs.emplace_back(internal_page->KeyAt(i),
//     internal_page->ValueAt(i));
//   }

//   // 找到新键的插入位置
//   auto compare_first = [this](
//                            const std::pair<KeyType, page_id_t> &lhs,
...
//   for (int i = 0; i < split_index; i++) {
//     internal_page->Insert(all_pairs[i].first, all_pairs[i].second,
//     comparator_);
//   }
